...
if __name__ == "__main__":
        ...
        for i_a,dic in enumerate(classifiers.items()):
            ...
            for k in K: 
                ...
                if clf.__name__ == "AgglomerativeClustering":
                    ...
                    for i in range(0,n_init):
                        if i_a==0:
                            ...
                            c0= initial_centers[(k-true_k -1)*n_init + i]
                ...
                if y_best_sol is not None:
                    ...
                    if k==true_k:
                        acc[index]=  metrics.rand_score(y_best_sol, y_)
    ...
    df_s= pd.read_csv(ROOT+"/data/test/shilhouette.csv",header=None).set_index(names).drop(columns=0).insert(0,"Scenario",scenarios).to_csv(ROOT+"/data/test/shilhouette.csv")
    df_ch= pd.read_csv(ROOT+"/data/test/calinski_harabasz.csv",header=None).drop(columns=0).set_index(names).insert(0,"Scenario",scenarios).to_csv(ROOT+"/data/test/calinski_harabasz.csv")
    df_db= pd.read_csv(ROOT+"/data/test/davies_boulding.csv",header=None).drop(columns=0).set_index(names).insert(0,"Scenario",scenarios).to_csv(ROOT+"/data/test/davies_boulding.csv")
    df_gci= pd.read_csv(ROOT+"/data/test/gci.csv",header=None).drop(columns=0).set_index(names).insert(0,"Scenario",scenarios).to_csv(ROOT+"/data/test/gci.csv")
    """
