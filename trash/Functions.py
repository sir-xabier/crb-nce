...
def global_covering_index2(U,function='min',orness=0.5,perc=5,mode=1,path=None):
    if mode:
...
def conds_score7(gci_,id,u,p=None,c=None,b=None):
    ...
    r_d=d[:-1]/d[1:] # ratio diferencias max_K-2
    r_d2=d2[:-1]/d2[1:] # ratio diferencias 2 max_K-3
    #r_l=d/gci_[:-1] # ratio relativo max_K-1
    #r_r=r_l[:-1]/r_l[1:] # ratio de ratios relativos max_K-2
    #r_e=p_e[:-1]/p_e[1:] # ratio proporciones cubiertas max_K-2
   
    #am_r_d=np.argmax(r_d)
    #am_r_e=np.argmax(r_e)
    #am_r_r=np.argmax(r_r)
    #am_r_d2=np.argmax(r_d2)
    ...
    if p is None and c is None:
        ...
        for i in range(1,k-3):
            ...
            if p_e[i-1] > u[0]: pts[i]+=1
                
            if r_d[i-1] > u[1]: pts[i]+=1

            if p_e_m > u[2]: pts[i]+=1

            #condicion sobre valores restantes de la 2a dif
            if m_d2 > u[3]: pts[i]+=1
            
            if abs(r_d2[i-1]) > u[4]: pts[i]+=1
                
            #ratio de 1a dif actual respecto a max de dif restantes
            if c_d[i-1] > u[5]: pts[i]+=1
                
            #ratio de 2a dif actual respecto a min de 2as dif restantes
            if c_d2[i-1] > u[6]: pts[i]+=1
            
        a=np.argmax(c_d)    
        am_c_d2=np.argmax(c_d2)
        
        if am_c_d2==a and c_d[a] > u[7]: pred=a+2
        else: 
            pred=np.argmax(pts)+1
        ...
        for i in range(1,k-3):
            ...
            if p_e[i-1] > u[0]: pts[i]+=p[0]
                
            if r_d[i-1] > u[1]: pts[i]+=p[1]

            if p_e_m > u[2]: pts[i]+=p[2]

            #condicion sobre valores restantes de la 2a dif
            if m_d2 > u[3]: pts[i]+=p[3]
            
            if abs(r_d2[i-1]) > u[4]: pts[i]+=p[4]
                
            #ratio de 1a dif actual respecto a max de dif restantes
            if c_d[i-1] > u[5]: pts[i]+=p[5]
                
            #ratio de 2a dif actual respecto a min de 2as dif restantes
            if c_d2[i-1] > u[6]: pts[i]+=p[6]
        ...
        if am_c_d2==a and c_d[a] > u[7]: pts[a+1]+=p[7]
        ...
        if c[7] and am_c_d2==a and c_d[a] > u[7]: pred=a+2
        else: 
            pred=np.argmax(pts)+1
...
def conds_score6(gci_,id,u,p=None,c=None,b=None):
...
def conds_score5(gci_,id,u,p=None,c=None,b=None):
    ...
    if p is None and c is None:
        ...
        if am_c_d2==a and c_d[a] > u[7]: pred=a+2
            ...
            if am_c_d2==a and am_r_d2==a and a+1 < k-3: pts[a+1]+=1
            if len(np.unique([a,am_c_d2,am_r_r,am_r_d,am_r_e]))==1 and a+1 < k-3: pts[a+1]+=1
            pred=np.argmax(pts)+1
        ...
        if am_c_d2==a and c_d[a] > u[7]: pred=a+2
            ...
            if am_c_d2==a and am_r_d2==a and a+1 < k-3: pts[a+1]+=p[7]
            if len(np.unique([a,am_c_d2,am_r_r,am_r_d,am_r_e]))==1 and a+1 < k-3: pts[a+1]+=p[8]
            pred=np.amax(np.asarray(pts==np.amax(pts)).nonzero())+1
    
    elif c is not None:
        pts[0]=np.amax([np.sum(c[:9])-(b[0]*1+b[1]*2),1])
        ...
        if c[9] and am_c_d2==a and c_d[a] > u[7]: pred=a+2
        else: 
            if c[7] and am_c_d2==a and am_r_d2==a and a+1 < k-3: pts[a+1]+=1
            if c[8] and len(np.unique([a,am_c_d2,am_r_r,am_r_d,am_r_e]))==1 and a+1 < k-3: pts[a+1]+=1
            pred=np.argmax(pts)+1
...
def conds_score4(gci_,id,u,p=None,c=None,b=None):
    ...
    if p is None and c is None:
        ...
        for i in range(1,k-3):
            ...
            if r_d[i-1] > u[0]: pts[i]+=1
                
            if r_l[i-1] > u[1]: pts[i]+=1

            if r_r[i-1]> u[2]: pts[i]+=1

            if p_n > u[3]: pts[i]+=1
            
            if p_e_m > u[4]: pts[i]+=1

            #condicion sobre valores restantes de la 2a dif
            if m_d2 > u[5]: pts[i]+=1
            
            if r_e[i-1] > u[6]: pts[i]+=1
            
            if abs(r_d2[i-1]) > u[7]: pts[i]+=1
                
            #ratio de 1a dif actual respecto a max de dif restantes
            if c_d[i-1] > u[8]: pts[i]+=1
                
            #ratio de 2a dif actual respecto a min de 2as dif restantes
            if c_d2[i-1] > u[9]: pts[i]+=1
        
        if len(np.unique([am_r_r,am_r_d,am_r_e]))==1 and np.amin([r_r[am_r_r],r_d[am_r_d],r_e[am_r_e]])>=u[10]:
            if am_r_r+1 < k-3: pts[am_r_r+1]+=1  
        
        am_c_d=np.argmax(c_d)    
        am_c_d2=np.argmax(c_d2)
        if len(np.unique([am_c_d,am_c_d2,am_r_d2]))==1:
            pred=am_c_d+2
        else:
            pred=np.argmax(pts)+1
        ...
        for i in range(1,k-3):
            ...
            if r_d[i-1] > u[0]: pts[i]+=p[0]
                
            if r_l[i-1] > u[1]: pts[i]+=p[1]

            if r_r[i-1]> u[2]: pts[i]+=p[2]

            if p_n > u[3]: pts[i]+=p[3]
            
            if p_e_m > u[4]:pts[i]+=p[4]

            #condicion sobre valores restantes de la 2a dif
            if m_d2 > u[5]: pts[i]+=p[5]
            
            if r_e[i-1] > u[6]: pts[i]+=p[6]
            
            if abs(r_d2[i-1]) > u[7]: pts[i]+=p[7]
                
            #ratio de 1a dif actual respecto a max de dif restantes
            if c_d[i-1]> u[8]: pts[i]+=p[8]
                
            #ratio de 2a dif actual respecto a min de 2as dif restantes
            if c_d2[i-1] >u[9]: pts[i]+=p[9]
        ...
        if len(np.unique([am_r_r,am_r_d,am_r_e]))==1 and np.amin([r_r[am_r_r],r_d[am_r_d],r_e[am_r_e]])>=u[10]:
            if am_r_r+1 < k-3: 
                pts[am_r_r+1]+=p[10]  
                if np.abs(pts[am_r_r+1]-1.0) < 1e-15: pts[am_r_r+1]=1.0
        ...
        pts[0]=np.amax([np.sum(c[:11])-(b[0]*1+b[1]*2),1])
        ...
        for i in range(1,k-3):
            ...
            if r_d[i-1] > u[0] and c[0]==1: pts[i]+=1
                
            if r_l[i-1] > u[1] and c[1]==1: pts[i]+=1

            if r_r[i-1]> u[2] and c[2]==1:
                pts[i]+=1

            if p_n > u[3] and c[3]==1: pts[i]+=1
            
            if p_e_m > u[4] and c[4]==1: pts[i]+=1

            #condicion sobre valores restantes de la 2a dif
            if m_d2 > u[5] and c[5]==1: pts[i]+=1
            
            if r_e[i-1] > u[6] and c[6]==1: pts[i]+=1
            
            if abs(r_d2[i-1]) > u[7] and c[7]==1: pts[i]+=1
                
            #ratio de 1a dif actual respecto a max de dif restantes
            if c_d[i-1] > u[8] and c[8]==1: pts[i]+=1
                
            #ratio de 2a dif actual respecto a min de 2as dif restantes
            if c_d2[i-1] > u[9] and c[9]==1: pts[i]+=1
            
        if c[10]==1 and len(np.unique([am_r_r,am_r_d,am_r_e]))==1 and np.amin([r_r[am_r_r],r_d[am_r_d],r_e[am_r_e]])>=u[10]:
            if am_r_r+1 < k-3: pts[am_r_r+1]+=1  

        
        am_c_d=np.argmax(c_d)    
        am_c_d2=np.argmax(c_d2)
        if c[11]==1 and len(np.unique([am_c_d,am_c_d2,am_r_d2]))==1:
            pred=am_c_d+2
        else:
            pred=np.argmax(pts)+1
...
def conds_score3(gci_,id,u,p=None,c=None,b=None):
