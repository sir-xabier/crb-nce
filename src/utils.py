...
allow the distance matrix D in the fit method to be passed as an argument instead 
of being computed inside the algorithm.
'''
...
class KMedoids(BaseEstimator, ClusterMixin, TransformerMixin):
    ...
    def _initialize_medoids(self, D, n_clusters, random_state_, X=None):
        ...
        if hasattr(self.init, "__array__"):  # Pre assign cluster
            ...
            medoids = np.array([np.where((X == c).all(axis=1))[0][0] for c in self.init])
