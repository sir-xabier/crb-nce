...
df_s= pd.read_csv(ROOT+"/data/test/shilhouette.csv",index_col=0)
df_ch= pd.read_csv(ROOT+"/data/test/calinski_harabasz.csv",index_col=0)
df_db= pd.read_csv(ROOT+"/data/test/davies_boulding.csv",index_col=0)
df_gci= pd.read_csv(ROOT+"/data/test/gci.csv",index_col=0)
df_y= pd.read_csv(ROOT+"/data/test/y.csv",header=None)

gen_pars="P50_G500_W35_M0.1_T15_R12_S1480"
train=""
#train=""
...
df=pd.DataFrame(columns=["s","ch","db","gci_c","gci_a","gci_se","gci_si","y"],index=names)
df
...
df_.drop(columns=["seed","dimensions","N","K","scenario"],axis=1).groupby(["dataset","algorithm"]).agg(acc).to_csv(ROOT+"/out_files/Results_acc_dataset_algorithm_"+gen_pars+train+".xlsx")
